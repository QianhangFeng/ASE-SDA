...
def evaluate_metrics_from_lists(predictions: List[str],
    ...
    unique_id = f'{random.randint(0, 1000000)}_{datetime.now()}'
...
def new_evaluate_metrics(prediction_file: Union[str, Path, List[Dict[str, str]]],
    ...
    all_pred_captions = predictions

    # fense_eval = Evaluator(device=device, sbert_model=None, batch_size=batch_size)
    fense_eval = Evaluator(device=device, batch_size=batch_size)
    fl_err = []
    fense_scores = []
    for pred_cap, gt in zip(all_pred_captions, ground_truths):
        score, has_err, _, _ = fense_eval.sentence_score(pred_cap, gt, return_error_prob=True)
        fense_scores.append(score)
        fl_err.append(has_err)
    ...
    total_metrics['FENSE'] = {'score':np.mean(fense_scores), 'scores': fense_scores}
